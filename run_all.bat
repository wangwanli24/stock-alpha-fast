...
python funB_multi_factor_ai\generate_factors.py
if %errorlevel% neq 0 (
...
if %errorlevel% neq 0 (
    echo [ERROR] Prediction failed. Please check batch_predict.py.
    pause
    exit /b
)
...
python funB_multi_factor_ai\batch_predict.py
if %errorlevel% neq 0 (
    echo [ERROR] Top10 generation failed. Please check batch_predict.py
    pause
    exit /b
)

REM Step 5: Generate Top10 recommendation and heatmap
echo [%TIME%] Generating Top10 recommendation...
