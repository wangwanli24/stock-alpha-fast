...
def generate_top10(input_path: str, output_path: str):
    ...
    valid_stocks = basic_info[~basic_info['name'].str.contains('ST|退')]
    valid_ts_codes = set(valid_stocks['ts_code'])
    df = df[df['ts_code'].isin(valid_ts_codes)]

    # ✅ 剔除成交金额小于10亿的股票（单位为万元）
    if "amount" in df.columns:
        df["成交金额(亿)"] = df["amount"] / 1e4
        df = df[df["成交金额(亿)"] >= 10]
    ...
    df["模型信心分"] = df["模型信心分"] if "模型信心分" in df.columns else (df["预测概率"] * 100).round(2)
    ...
    df["预测得分"] = (df["return_3"] * 100).round(2)  # 可选加权因子
    ...
    # ====== 导出字段 ======
    columns_to_save = [
        "ts_code", "trade_date", "预测概率", "预测得分", "建议",
        "return_3", "return_std_5", "amount_ratio",
        "模型信心分", "波动率", "综合评分", "amount"
    ]
...
if __name__ == "__main__":
    ...
    predict_date_str = latest_file.split("_")[-1].split(".")[0]
